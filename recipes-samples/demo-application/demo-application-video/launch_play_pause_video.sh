...
function pty_exec() {
    ...
    echo "/usr/local/demo/bin/play_pause-gtk-player -w $SCREEN_WIDTH -h $SCREEN_HEIGHT $cmd"
    if [ $pty -eq 0 ]; then
        cmd=$(echo $cmd | sed "s#\"#'#g")
        event_cmd=$(echo /usr/local/demo/bin/play_pause-gtk-player -w $SCREEN_WIDTH -h $SCREEN_HEIGHT $cmd)
        eval $event_cmd > /dev/null 2>&1
    else
        # no pty
        echo "NO PTY"
        event_cmd=$(echo /usr/local/demo/bin/touch-event-gtk-player -w $SCREEN_WIDTH -h $SCREEN_HEIGHT $cmd)
...
then
    VIDEO_FILE=file:///usr/local/demo/media/ST19619_ST_Company_Video_16_9_EN_272p.webm
else
    VIDEO_FILE=file:///usr/local/demo/media/ST2297_visionv3.webm
fi
